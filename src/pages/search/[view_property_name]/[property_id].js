...
import GolfHomMobileMenuLogo from "../../../../public/images/Logo-mobile-menu.webp";
...
const ViewProperty = (props) => {
  ...
  useEffect(() => {
    const GetGolfCourseByLatLong = async () => {
      try {
        const GetGolfCourseRes = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL}/v1/golfcourse?latitude=${SpecificPropAPIData?.latitude}&longitude=${SpecificPropAPIData?.longitude}&distance=8`
        );
        if (GetGolfCourseRes.status === 200) {
          let gc = GetGolfCourseRes.data.data.slice(0, 5);
          setGolfCourseData(GetGolfCourseRes.data.data.slice(0, 5));

          setGcMarkers(gc.map(({ latitude, longitude }) => {
    ...
    if (SpecificPropAPIData?.latitude && SpecificPropAPIData?.longitude) {
  ...
  }, [SpecificPropAPIData?.latitude, SpecificPropAPIData?.longitude]);
  ...
  //   const gettoken = await axios.post(process.env.NEXT_PUBLIC_LIVE_API_TOKEN_URL, {
  //     grant_type: "client_credentials", client_id: process.env.NEXT_PUBLIC_CLIENT_ID, client_secret: process.env.NEXT_PUBLIC_CLIENT_SECTET
  //   }, { headers: { "Content-Type": "application/x-www-form-urlencoded" } }
  //   );
  //   if (gettoken.status == 200 && gettoken.data.access_token) {
    ...
    const GetPropertyById = async () => {
      try {
        const SpecificPropData = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL}/v1/property/getOne/${Params.property_id || UrlParamId
          }`
        );
        ...
        if (SpecificPropData.status === 200) {
          ...
          // console.log("SpecificPropData.data.response.externalPropertyType: ", SpecificPropData.data.response.externalPropertyType)
  ...
  const items = [
    {
      key: "1",
      label: `Overview`,
      children: <TabContentOverview data={SpecificPropAPIData} GolfCourseData={GolfCourseData} />,
    },
  ];
    ...
    const CheckPriceRental = async () => {
      ...
      try {
        const CheckAvailRes = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL}/v1/rentalunited/price?id=${Params.property_id
          ...
          const CheckPriceRental = async () => {
            ...
            try {
              const CheckAvailRes = await axios.get(
                ...
                }/v1/rentalunited/priceBreakdown?id=${Params.property_id
    ...
    if (PropertyType === "Nextpax") {
      const data = await axios({
        ...
        if (res.status === 200) {
          ...
          // console.log("AvailDataNextpax: ", AvailDataNextpax)
      ...
      if (PropertyType === "Nextpax") {
        ...
        const CheckPriceRental = async () => {
          ...
          try {
            const CheckAvailRes = await axios.get(
              `${process.env.NEXT_PUBLIC_API_URL
              }/v1/rentalunited/priceBreakdown?id=${Params.property_id}&from=${BookingDate[0]
                  ...
                  >
                    ...
                    <div className={ViewPropertyCss.paymentMainBlock}>
                      <div className={ViewPropertyCss.paymentLogoPrice}>
                        <div>
                          <NextImage src={GolfHomMobileMenuLogo} alt="logo"></NextImage>
                        </div>
                        <div>
                          <div>
        ...
        <div className={ViewPropertyCss.map_section}>
          <Container>
            <div className={ViewPropertyCss.map_section_main_container}>
              <Map data={[SpecificPropAPIData]} gcMarkers={gcMarkers} />
            </div>
          </Container>
        </div>
