...
import NoDataFound from "@/components/host/Common/NoDataFound";
...
const HostProperties = () => {
  ...
  const onSave = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.patch(`/v1/host/property/${propertyId}`, body, {
        headers: { Authorization: `Bearer ${session?.user?.token}` }
      });

      if (response.status === 201) {

        successtoast({ message: "Property has been updated successfully" });
  ...
  // Get Initial Data
  useEffect(() => {
    slug && session?.user?.token && getProperty();
  }, [slug]);
  ...
  const getProperty = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        const { id, arrival_instructions, property_info, policies, pricing, externalPropertyId, externalPropertyType, host_id, ownerId, ownerName, slug, updatedAt, createdAt, availability, paymentMethods, ...property_data } = response.data.response;
        const paymentMethods_parsed = paymentMethods.map(p => ({ name: p }))
        setPropertyId(id);

        setData({ ...arrival_instructions, ...property_info, ...policies, ...pricing, availability, ...property_data, paymentMethods: paymentMethods_parsed })
      }
    } catch (err) {
